...
public class VistaPropia extends View {
    ...
    int radio_MHN = 80;
    int radioColision_MHN = 45;

    int puntuacion_MHN = 0;

    // - balas jugador - //
    int maxBalas_MHN = 10;
    int[] balaX_MHN = new int[maxBalas_MHN];
    int[] balaY_MHN = new int[maxBalas_MHN];
    boolean[] balaActiva_MHN = new boolean[maxBalas_MHN];
    int balaRadio_MHN = 15;
    int velocidadBala_MHN = 30;
    boolean dedoPulsado_MHN = false;
    long ultimoDisparoJugador_MHN = 0;
    int cadenciaDisparo_MHN = 300;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //- limitador velocidad -//
        if (aumentoVelocidad_MHN > 15) {
            aumentoVelocidad_MHN = 15;
        }
...
//            enemigoY += velocidadY;
        ...
        // - disparo auto - //
        if (juegoIniciado_MHN && !gameOver_MHN && dedoPulsado_MHN) {
            intentarDispararJugador_MHN();
        }
        // - Balas del jugador - //
        for (int i = 0; i < maxBalas_MHN; i++) {
            if (balaActiva_MHN[i]) {
                pincel_MHN.setARGB(255, 250, 255, 250);
                canvas.drawRect(balaX_MHN[i] - 30, balaY_MHN[i] - 8, balaX_MHN[i] + 30, balaY_MHN[i] + 8, pincel_MHN);

                // - Movimiento bala - //
                balaX_MHN[i] = balaX_MHN[i] - velocidadBala_MHN;

                // - Desactivar bala al salir de pantalla - //
                if (balaX_MHN[i] + balaRadio_MHN < 0) {
                    balaActiva_MHN[i] = false;
                }
            }
        }
        ...
        for (int i = 0; i < enemigos_MHN; i++) {
            if (enemigosActivos_MHN[i]) {
                ...
                for (int j = 0; j < maxBalas_MHN; j++) {
                    if (balaActiva_MHN[j]) {
                        int distBalaX = balaX_MHN[j] - enemigoX_MHN[i];
                        int distBalaY = balaY_MHN[j] - enemigoY_MHN[i];
        ...
        if (gameOver_MHN) {
            ...
            if (!sonidoGameOver_MHN) {
                ...
                // para poder parar la música al morir, hay que notificar al gameactivity para que la pare
                if (getContext() instanceof GameActivity) {
                    ((GameActivity) getContext()).pararMusica_MHN();
                }
            }
            //------ Pantalla GAME OVER ------- //
    ...
    private void intentarDispararJugador_MHN() {
        long tiempoActual_MHN = System.currentTimeMillis();

        if (tiempoActual_MHN - ultimoDisparoJugador_MHN >= cadenciaDisparo_MHN) {

            for (int i = 0; i < maxBalas_MHN; i++) {
                if (!balaActiva_MHN[i]) {
                    balaX_MHN[i] = posX_MHN - radio_MHN;
                    balaY_MHN[i] = posY_MHN;
                    balaActiva_MHN[i] = true;
                    ultimoDisparoJugador_MHN = tiempoActual_MHN;
    ...
    private void reiniciarJuego_MHN() {
        ...
        if (getContext() instanceof GameActivity) {
            GameActivity miActividad = (GameActivity) getContext();
            MediaPlayer mp = miActividad.getMediaPlayer_MHN();

            if (mp != null) {
                mp.seekTo(0); // Vuelve la música al principio
                mp.start();
            }
        }
    }
